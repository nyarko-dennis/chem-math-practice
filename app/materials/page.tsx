...
import {
  materialsQuestions,
  MATERIALS_TOPIC_LABELS,
  MaterialsQuestion,
  MaterialsTopic,
} from '@/lib/materialsQuestions';
import {
  materialsDrills,
  MATERIALS_DRILL_LABELS,
  MaterialsDrill,
  RubricPoint,
...
type SelectedTopics = Record<MaterialsTopic, boolean>;

const DEFAULT_TOPICS: SelectedTopics = {
  introClassification: true,
  atomicCrystal: true,
  solidificationDefects: true,
  mechanicalTesting: true,
  ironCarbonHeat: true,
  steelProduction: true,
  ferrousSteels: true,
  corrosionAdvanced: true,
};
...
function shuffleMCQ(q: MaterialsQuestion): ShuffledMCQ | null {
...
export default function MaterialsPage() {
  ...
  useEffect(() => {
    setMounted(true);
    const session = getActiveSession('materials');
    if (session) {
  ...
  // quick mode state
  const [questions, setQuestions] = useState<MaterialsQuestion[]>([]);
  ...
  // drill mode state
  const [drills, setDrills] = useState<MaterialsDrill[]>([]);
  ...
  const selectedTopicList = useMemo(
    () => (Object.keys(topics) as MaterialsTopic[]).filter((t) => topics[t]),
    [topics],
  );

  const quickCountsByTopic = useMemo(() => {
    const counts: Partial<Record<MaterialsTopic, number>> = {};
    materialsQuestions.forEach((q) => {
  ...
  const startQuick = () => {
    ...
    const pool = materialsQuestions.filter((q) => selectedTopicList.includes(q.topic));
    if (pool.length === 0) {
  ...
  const startDrill = () => {
    ...
    const pool = materialsDrills.filter((d) => selectedTopicList.includes(d.topic));
    if (pool.length === 0) {
  ...
  const isQuickCorrect = (q: MaterialsQuestion): boolean => {
  ...
  const drillSelfScore = (d: MaterialsDrill): number => {
  ...
  useEffect(() => {
    if (started && !finished) {
      ...
      saveActiveSession('materials', activeState);
    }
  }, [
    started,
    finished,
        ...
        <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-lg">
          ...
          {hasSavedSession && (
            <div className="bg-cyan-50 border border-cyan-200 rounded-lg p-4 mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shadow-sm">
              <div>
                <h3 className="font-semibold text-cyan-800 text-sm">Session in Progress</h3>
                <p className="text-xs text-cyan-700">You have a saved {savedSessionMode === 'quick' ? 'Quick Practice' : 'Subjective Drill'} session.</p>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={loadSavedSession}
                  className="bg-cyan-600 hover:bg-cyan-700 text-white text-xs font-semibold px-3 py-1.5 rounded-md transition-colors shadow-sm"
                >
                  Resume
                </button>
                <button
          ...
          <p className="text-sm text-slate-500 mb-4">
            {mode === 'quick'
              ? 'MCQ and True/False questions across all Engineering Materials topics, with instant feedback and rationale.'
            ...
            {(Object.keys(MATERIALS_TOPIC_LABELS) as MaterialsTopic[]).map((t) => (
              <label key={t} className="flex items-center justify-between cursor-pointer gap-3">
                ...
                <input
                  ...
                  className="w-5 h-5 accent-cyan-600 flex-shrink-0"
                />
              </label>
            ))}
          </div>
          ...
          <div className="pt-4 border-t border-slate-100 mb-6">
            ...
            <input
              ...
              className="w-full p-2 border border-slate-300 rounded-md focus:ring-2 focus:ring-cyan-500 focus:outline-none"
            />
  ...
  if (finished) {
    ...
    drills.forEach((d) => {
      const key = d.topic;
      if (!topicScores[key]) {
        topicScores[key] = { earned: 0, total: 0, label: MATERIALS_TOPIC_LABELS[d.topic] };
      }
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white p-8 rounded-xl shadow-lg">
            ...
            <div className="bg-cyan-50 border border-cyan-200 rounded-lg p-6 mb-6 text-center">
              <div className="text-sm uppercase tracking-wide text-cyan-600 font-semibold mb-1">
                Total self-score
              </div>
              <div className="text-4xl font-bold text-cyan-700">
                {earnedMarks} / {totalMarks}
              </div>
              <div className="text-cyan-600 mt-1">{pct}%</div>
            </div>
                  ...
                  >
                    <div>
                      <div className="font-medium text-slate-800 text-sm">
                        Drill {i + 1} · {MATERIALS_DRILL_LABELS[d.drillKind]}
                      </div>
                    ...
                    <div
                      className={`font-bold text-sm ml-3 whitespace-nowrap ${
                        passed ? 'text-green-600' : 'text-cyan-600'
                      }`}
                    >
                      {score} / {d.marks}
                    </div>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white p-8 rounded-xl shadow-lg mb-6">
            <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
              <p className="text-cyan-600 text-xs uppercase tracking-wide font-semibold">
                {MATERIALS_DRILL_LABELS[d.drillKind]} · {d.marks} marks
              </p>
            </div>
            ...
            {!isRevealed && (
              <>
                ...
                <textarea
                  ...
                  className="w-full p-3 border border-slate-300 rounded-md focus:ring-2 focus:ring-cyan-500 focus:outline-none min-h-[200px] text-sm text-slate-800"
                />
                <div className="flex justify-end mt-4">
                  <button
                    onClick={revealDrill}
                    className="bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-3 px-8 rounded-lg shadow-md transition-transform active:scale-95"
                  >
          ...
          {isRevealed && (
            <div className="bg-white p-8 rounded-xl shadow-lg mb-6">
              <div className="mb-6 p-4 bg-cyan-50 border border-cyan-200 rounded-lg">
                <div className="text-xs uppercase tracking-wide text-cyan-600 font-semibold mb-2">
                  Model answer
                </div>
              ...
              <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
                <div>
                  <div className="text-xs uppercase tracking-wide text-cyan-600 font-semibold">
                    Self-score
                  </div>
                    ...
                    >
                      <div className="flex items-start gap-3 p-3">
                        <input
                          ...
                          className="w-5 h-5 mt-0.5 accent-cyan-600 cursor-pointer flex-shrink-0"
                        />
        ...
        <div className="bg-white p-8 rounded-xl shadow-lg mb-6">
          <div className="mb-6">
            <div className="flex flex-wrap items-center gap-2 mb-3">
              <span className="text-cyan-600 text-xs uppercase tracking-wide font-semibold">
                {MATERIALS_TOPIC_LABELS[currentQ.topic]}
              </span>
          ...
          <div className="space-y-2 mb-6">
            {currentQ.type === 'mcq' && sh ? (
              sh.displayChoices.map((choice, idx) => {
                ...
                } else if (isSelected) cls += ' border-cyan-500 bg-cyan-50';
                else cls += ' border-slate-200 hover:border-cyan-300 hover:bg-slate-50';
                return (
                  <button
                    key={idx}
                    disabled={hasChecked}
          ...
          <div className="flex justify-end gap-3">
            {!hasChecked ? (
              <button
                ...
                className="bg-cyan-600 hover:bg-cyan-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white font-bold py-3 px-8 rounded-lg shadow-md transition-transform active:scale-95"
              >
                Check Answer
              </button>
            ) : (
