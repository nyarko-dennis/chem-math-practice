...
} from '@/lib/pharmacologyQuestions';
import {
  PharmDrill,
  PharmDrillKind,
  PHARM_DRILL_LABELS,
...
type SelectedTopics = Record<PharmTopic, boolean>;
type SelectedKinds = Record<PharmDrillKind, boolean>;
...
const DEFAULT_KINDS: SelectedKinds = {
  defineTerms: true,
  compareConcepts: true,
  mechanismOfAction: true,
  adverseEffects: true,
  patientCounselling: true,
  caseManagement: true,
  fiveRights: true,
  drugCalc: true,
};
...
function shuffleMCQ(q: PharmQuestion): ShuffledMCQ | null {
...
export default function PharmacologyPage() {
  ...
  const [questions, setQuestions] = useState<PharmQuestion[]>([]);
  ...
  const [drills, setDrills] = useState<PharmDrill[]>([]);
  ...
  const selectedTopicList = useMemo(
    () => (Object.keys(topics) as PharmTopic[]).filter((t) => topics[t]),
    [topics],
  );

  const selectedKindList = useMemo(
    () => (Object.keys(kinds) as PharmDrillKind[]).filter((k) => kinds[k]),
    [kinds],
  );
  ...
  const isQuickCorrect = (q: PharmQuestion): boolean => {
  ...
  const drillSelfScore = (d: PharmDrill): number => {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-lg">
          <CourseTabs courseId="pharmacology" active="practice" />
          ...
          <p className="text-sm text-slate-500 mb-4">
            {mode === 'quick'
              ? 'MCQ and True/False questions across all pharmacology topics with instant feedback.'
          ...
          {mode === 'drill' && (
            <>
              {/* Drill type checkboxes */}
              <div className="space-y-3 mb-6 max-h-64 overflow-y-auto pr-1">
                {(Object.keys(PHARM_DRILL_LABELS) as PharmDrillKind[]).map((k) => (
                  <label key={k} className="flex items-center justify-between cursor-pointer">
                    <span className="font-medium text-slate-700 text-sm">
                      {PHARM_DRILL_LABELS[k]}
  ...
  if (finished) {
    ...
    drills.forEach((d) => {
      const key = d.topic;
      if (!topicScores[key]) {
        topicScores[key] = { earned: 0, total: 0, label: PHARM_TOPIC_LABELS[d.topic] };
      }
      ...
      <div className="min-h-screen bg-slate-50 p-4">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white p-8 rounded-xl shadow-lg">
            ...
            <div className="bg-rose-50 border border-rose-200 rounded-lg p-6 mb-6 text-center">
              <div className="text-sm uppercase tracking-wide text-rose-600 font-semibold mb-1">
                Total self-score
              </div>
              <div className="text-4xl font-bold text-rose-700">
                {earnedMarks} / {totalMarks}
              </div>
              <div className="text-rose-600 mt-1">{pct}%</div>
            </div>
                  ...
                  >
                    <div>
                      <div className="font-medium text-slate-800 text-sm">
                        Drill {i + 1} · {PHARM_DRILL_LABELS[d.drillKind]}
                      </div>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white p-8 rounded-xl shadow-lg mb-6">
            <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
              <p className="text-rose-600 text-xs uppercase tracking-wide font-semibold">
                {PHARM_DRILL_LABELS[d.drillKind]} · {d.marks} marks
              </p>
              {isRevealed && (
                <button
                  onClick={() =>
            ...
            {!isRevealed && (
              <>
                ...
                <textarea
                  ...
                  className="w-full p-3 border border-slate-300 rounded-md focus:ring-2 focus:ring-rose-500 focus:outline-none min-h-[200px] text-sm text-slate-800"
                />
                <div className="flex justify-end mt-4">
                  <button
                    onClick={revealDrill}
                    className="bg-rose-600 hover:bg-rose-700 text-white font-bold py-3 px-8 rounded-lg shadow-md transition-transform active:scale-95"
                  >
                    Reveal Model Answer
                  </button>
                </div>
          ...
          {/* Rubric and Model Answer after reveal */}
          {isRevealed && (
            <div className="bg-white p-8 rounded-xl shadow-lg mb-6">
              <div className="mb-6 p-4 bg-rose-50 border border-rose-200 rounded-lg">
                <div className="text-xs uppercase tracking-wide text-rose-600 font-semibold mb-2">
                  Model answer
                </div>
                    ...
                    >
                      <div className="flex items-start gap-3 p-3">
                        <input
                          ...
                          className="w-5 h-5 mt-0.5 accent-rose-600 cursor-pointer flex-shrink-0"
                        />
              ...
              {d.referenceSections && d.referenceSections.length > 0 && (
                <div className="mb-6 p-4 bg-slate-50 rounded-lg">
                  <div className="text-xs uppercase tracking-wide text-slate-500 font-semibold mb-2">
                    Reference (docs/pharmacology/pharmacology.md)
                  </div>
                  <div className="space-y-1">
                    {d.referenceSections.map((s) => (
                      <div key={s} className="text-sm text-rose-700 font-mono text-xs">
                        → {s}
                      </div>
                    ))}
                  </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 font-sans">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white p-8 rounded-xl shadow-lg mb-6">
          <div className="mb-6">
            <p className="text-rose-600 text-xs mb-3 uppercase tracking-wide font-semibold">
              {PHARM_TOPIC_LABELS[currentQ.topic]} · {currentQ.type === 'mcq' ? 'Multiple Choice' : 'True / False'}
            </p>
          ...
          <div className="space-y-2 mb-6">
            {currentQ.type === 'mcq' && sh ? (
              sh.displayChoices.map((choice, idx) => {
                ...
                else cls += ' border-slate-200 hover:border-rose-300 hover:bg-slate-50';
                return (
                  <button
                    key={idx}
                    disabled={hasChecked}
          ...
          <div className="flex justify-end gap-3">
            {!hasChecked ? (
              <button
                ...
                className="bg-rose-600 hover:bg-rose-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white font-bold py-3 px-8 rounded-lg shadow-md transition-transform active:scale-95"
              >
                Check Answer
              </button>
            ) : (
