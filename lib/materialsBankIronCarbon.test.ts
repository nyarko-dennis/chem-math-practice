...
test('bank has exactly 30 items with sequential prefixed ids', () => {
  assert.equal(ironCarbonHeatQuestions.length, 30);
  ironCarbonHeatQuestions.forEach((q, i) => {
    assert.equal(q.id, `ih-${String(i + 1).padStart(3, '0')}`);
  });
});

test('every item is well-formed and correctly tagged', () => {
  for (const q of ironCarbonHeatQuestions) {
    assert.equal(q.topic, 'ironCarbonHeat');
...
test('mix is roughly 22 MCQ / 8 TF and correctIndex varies', () => {
  const mcq = ironCarbonHeatQuestions.filter((q) => q.type === 'mcq');
  const tf = ironCarbonHeatQuestions.filter((q) => q.type === 'tf');
