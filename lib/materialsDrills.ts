...
export const materialsDrills: MaterialsDrill[] = [
  ...
  },
  {
    ...
    modelAnswer:
      ...
      'The eyeglass frame is a smart material, most likely a shape-memory alloy such as nitinol. It is bent out of shape like an ordinary metal frame, but once it warms past a set transition temperature (from being worn against the skin), it senses that temperature change and straightens itself back to its original trained shape automatically, with no lever, spring, or outside switch needed - the defining behavior of a smart material rather than a plain metal.',
  },
  {
    id: 'mdr-003',
    topic: 'introClassification',
  ...
  },
  {
    ...
    type: 'drill',
    prompt:
      'Explain how a tensile test is carried out and describe THREE things the resulting stress-strain curve reveals about the material.',
    marks: 9,
    rubric: [
    ...
    modelAnswer:
      ...
      'Finally, the curve ends abruptly at the point where the sample actually snaps in two, and the total amount of stretch the curve reached by that final point is a direct measure of the material\'s ductility - a long curve before fracture means a very ductile material, while a short one that ends soon after the elastic region means a brittle one.',
  },
  ...
  },
  {
    ...
    modelAnswer:
      'Tempering exists because of a problem that quenching itself creates. Quenching hardens steel by cooling it so fast that carbon atoms get trapped inside the iron lattice, forming martensite, an extremely hard structure that also carries a great deal of internal stress. In this freshly quenched state the steel is usually too brittle to be put straight into use - it would crack rather than absorb a sudden knock.\n\n' +
      'Tempering fixes this by reheating the already-hardened steel to a temperature that stays below its lower critical point (well below the temperature originally used for hardening), holding it there for a set length of time, and then letting it cool, usually just in still air rather than by any special quenching. Because the reheat stays below the critical point, no new austenite forms and the basic hardened structure is not undone, only adjusted.\n\n' +
