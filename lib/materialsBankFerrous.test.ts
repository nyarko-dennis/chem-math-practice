...
test('bank has exactly 30 items with sequential prefixed ids', () => {
  assert.equal(ferrousSteelsQuestions.length, 30);
  ferrousSteelsQuestions.forEach((q, i) => {
    assert.equal(q.id, `fs-${String(i + 1).padStart(3, '0')}`);
  });
});

test('every item is well-formed and correctly tagged', () => {
  for (const q of ferrousSteelsQuestions) {
    assert.equal(q.topic, 'ferrousSteels');
...
test('mix is roughly 22 MCQ / 8 TF and correctIndex varies', () => {
  const mcq = ferrousSteelsQuestions.filter((q) => q.type === 'mcq');
  const tf = ferrousSteelsQuestions.filter((q) => q.type === 'tf');
