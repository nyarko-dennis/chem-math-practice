...
test('bank has exactly 30 items with sequential prefixed ids', () => {
  assert.equal(steelProductionQuestions.length, 30);
  steelProductionQuestions.forEach((q, i) => {
    assert.equal(q.id, `sp-${String(i + 1).padStart(3, '0')}`);
  });
});

test('every item is well-formed and correctly tagged', () => {
  for (const q of steelProductionQuestions) {
    assert.equal(q.topic, 'steelProduction');
...
test('mix is roughly 22 MCQ / 8 TF and correctIndex varies', () => {
  const mcq = steelProductionQuestions.filter((q) => q.type === 'mcq');
  const tf = steelProductionQuestions.filter((q) => q.type === 'tf');
