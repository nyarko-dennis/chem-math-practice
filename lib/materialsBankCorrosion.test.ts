...
test('bank has exactly 30 items with sequential prefixed ids', () => {
  assert.equal(corrosionAdvancedQuestions.length, 30);
  corrosionAdvancedQuestions.forEach((q, i) => {
    assert.equal(q.id, `ca-${String(i + 1).padStart(3, '0')}`);
  });
});

test('every item is well-formed and correctly tagged', () => {
  for (const q of corrosionAdvancedQuestions) {
    assert.equal(q.topic, 'corrosionAdvanced');
...
test('mix is roughly 22 MCQ / 8 TF and correctIndex varies', () => {
  const mcq = corrosionAdvancedQuestions.filter((q) => q.type === 'mcq');
  const tf = corrosionAdvancedQuestions.filter((q) => q.type === 'tf');
