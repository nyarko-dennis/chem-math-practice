...
test('bank has exactly 30 items with sequential prefixed ids', () => {
  assert.equal(solidificationDefectsQuestions.length, 30);
  solidificationDefectsQuestions.forEach((q, i) => {
    assert.equal(q.id, `sd-${String(i + 1).padStart(3, '0')}`);
  });
});

test('every item is well-formed and correctly tagged', () => {
  for (const q of solidificationDefectsQuestions) {
    assert.equal(q.topic, 'solidificationDefects');
...
test('mix is roughly 22 MCQ / 8 TF and correctIndex varies', () => {
  const mcq = solidificationDefectsQuestions.filter((q) => q.type === 'mcq');
  const tf = solidificationDefectsQuestions.filter((q) => q.type === 'tf');
